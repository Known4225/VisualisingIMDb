...
char *list_to_string(list_t *list) { // turns a list to a (malloc'd) string (currently only supports char lists because laziness)
    char *toWrite = malloc(sizeof(char) * (1 + list -> length));
    int next = 0;
    for (int i = 0; i < list -> length; i++) {
        unitype item = list -> data[i];
        switch (list -> type[i]) {
            case 'i':
                // sprintf(toWrite, "%d", item.i);
            break;
            case 'u':
                // sprintf(toWrite, "%i", item.i);
            break;
            case 'f':
                // sprintf(toWrite, "%f", item.f);
            break;
            case 'd':
                // sprintf(toWrite, "%lf", item.d);
            break;
            case 'c':
                // char append;
                // sprintf(&append, "%c", item.c);
                toWrite[next] = item.c;
                next++;
            break;
            case 's':
                // sprintf(toWrite, "%s", item.s);
            break;
            case 'p':
                // sprintf(toWrite, "%p", item.p);
            break;
            case 'r':
                // sprintf(toWrite, "%s", list_to_string(item.r));
            break;
            case 'l':
                // sprintf(toWrite, "%lld", item.l);
            break;
            case 'h':
                // sprintf(toWrite, "%hi", item.h);
            break;
            case 'b':
                // sprintf(toWrite, "%hi", item.b);
            break;
            default:
                printf("print - type not recognized\n");
                return NULL;
