...
void sortTitles(visual *selfp, char *filename) {
    printf("opening %s\n", filename);
    visual self = *selfp;
    FILE *fp = fopen(filename, "w");
    /*
    sort based on number not ascii
    datasets[0] - titleBasics, titleRatings, or nameBasics
    */
    list_t *dataX = self.datasets -> data[0].r;
    printf("length: %d\n", dataX -> data[0].r -> length);
    list_t *rowlist = list_init();

    /* first gather 9 digiters */
    long long numLines = dataX -> data[0].r -> length;
    ...
    turtlePenDown();
    turtleUpdate();
    for (int i = 1; i < dataX -> data[0].r -> length; i++) {
        if (iters > nextThresh) {
        ...
        char *tempStr = dataX -> data[0].r -> data[i].s;
        if (strlen(tempStr) == 9) {
            list_clear(rowlist);
            for (int j = 0; j < dataX -> length; j++) {
                list_append(rowlist, dataX -> data[j].r -> data[i], 's');
            }
            char *toAdd = list_to_string_advanced(rowlist, "\t", 1);
            fprintf(fp, "%s", toAdd);
        }
    }
    /* then gather 10 digiters */
    numLines = dataX -> data[0].r -> length;
    nextThresh = numLines / selfp -> loadingSegmentsE;
    threshLoaded = 0;
    iters = 0;
    turtlePenColor(0, 0, 0);
...
void exportOnlyActors(visual *selfp, char *filename) {
    ...
    */
    ...
    for (int i = 1; i < titlePrincipals -> data[0].r -> length; i++) {
        ...
        if (strcmp(tempStr, "actor") == 0 || strcmp(tempStr, "actress") == 0 || strcmp(tempStr, "self") == 0) {
            list_clear(rowlist);
            for (int j = 0; j < titlePrincipals -> length; j++) {
                list_append(rowlist, titlePrincipals -> data[j].r -> data[i], 's');
            }
...
void exportNames(visual *selfp, char *filename) {
    ...
    visual self = *selfp;
    FILE *fp = fopen(filename, "w");
    /* package to actorActor associations
    datasets[0] - titlePrincipals
    */
    list_t *titlePrincipals = self.datasets -> data[0].r;
    printf("length: %d\n", titlePrincipals -> data[0].r -> length);
    ...
    for (int i = 0; i < 40000000; i++) {
        if (output -> data[i].r -> length > 0) {
            for (int k = 0; k < output -> data[i].r -> length; k += 2) {
                char *convert = malloc(4);
                sprintf(convert, "%03d", output -> data[i].r -> data[k].i);
                output -> data[i].r -> type[k] = 's';
                output -> data[i].r -> data[k].s = convert;
            }
            char *sweep = list_to_string_advanced(output -> data[i].r, ",", 1);
            char *toAdd = malloc(sizeof(char) * (strlen(sweep) + 1 + 11));
            char *nameID = malloc(11);
            nameID[0] = 'n';
            nameID[1] = 'm';
            sprintf(nameID + 2, "%08d", i);
            strcpy(toAdd, nameID);
            strcpy(toAdd + 10, "\t");
            strcpy(toAdd + 11, sweep);
            // printf("%d\n", i);
            fprintf(fp, "%s", toAdd);
            free(nameID);
            free(sweep);
    ...
    */
    ...
    for (int i = 0; i < 40000000; i++) {
        if (atoi(actorActor -> data[0].r -> data[j].s + 2) == i) {
            ...
            for (int k = 2; k < actorActor -> length; k++) {
                list_append(rowlist, actorActor -> data[k].r -> data[j], 's');
            }
...
int main(int argc, char *argv[]) {
    ...
    // exportNames(&self, "customSetStarter.tsv");
    // sortTitles(&self, "titlePrincipalsSorted.tsv");
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\titleBasicsSorted.tsv");

    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\titlePrincipalsSortedOnlyMovies.tsv");
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\titleBasicsSorted.tsv");
    // exportOnlyActors(&self, "titlePrincipalsSortedOnlyMovieActors.tsv");
    // exportNames(&self, "customSetStarter.tsv");


    // export(&self, "titlePrincipalsSortedOnlyMovies.tsv");
    // import(&self, "titlePrincipalsTruncated.tsv");
    ...
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\nameBasics.tsv");
    // sortTitles(&self, "nameBasicsSorted.tsv");

    // import(&self, "customSetStarter.tsv");
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\nameBasicsSorted.tsv");
    // gatherDoB(&self, "customSetDoB.tsv");


    // import(&self, "customSetNames.tsv");
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\titlePrincipalsSortedOnlyMovieActors.tsv");
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\titleRatingsSorted.tsv");
    // addRecognisability(&self, "customSetRecog.tsv");
    ...
    // import(&self, "customSetRecog.tsv");
    // import(&self, "D:\\Characters\\Information\\College\\GeeVis\\nameBasicsSorted.tsv");
    // addNameStrings(&self, "customSetNames.tsv");
